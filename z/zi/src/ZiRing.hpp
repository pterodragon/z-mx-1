...
public:
  ...
  void push2() {
    ...
    uint32_t size_ = align(Traits::size(*reinterpret_cast<const T *>(&ptr[8])));
    head += size_;
  ...
  int gc() {
    ...
    while (tail != head) {
      ...
      uint32_t n = align(Traits::size(*reinterpret_cast<const T *>(&ptr[8])));
      tail += n;
  ...
  int attach() {
	...
	tail += align(Traits::size(*reinterpret_cast<const T *>(&ptr[8])));
