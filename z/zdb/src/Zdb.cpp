    ...
    ZdbAnyPOD *pod, int type, ZdbRange range, int op, bool compress)
{
  ...
  this->send(ZiIOFn::Member<&ZdbAnyPOD_Send__::send>::fn(ZuMv(send)));
}
...
void ZdbAnyPOD_Send__::sent(ZiIOContext &io)
{
  ...
  if (m_compressed)
    io.init(ZiIOFn::Member<&ZdbAnyPOD_Send__::sent2>::fn(
	  io.fn.mvObject<ZdbAnyPOD_Send__>()),
	m_compressed->ptr(), rep.clen, 0);
  else if (range)
    io.init(ZiIOFn::Member<&ZdbAnyPOD_Send__::sent2>::fn(
	  io.fn.mvObject<ZdbAnyPOD_Send__>()),
	(void *)((const char *)m_pod->ptr() + range.off()), range.len(), 0);
  else
    sent3(io);
}
void ZdbAnyPOD_Send__::sent2(ZiIOContext &io)
{
  if ((io.offset += io.length) < io.size) return;
  sent3(io);
}
void ZdbAnyPOD_Send__::sent3(ZiIOContext &io)
{
    ...
    const Zdb_Msg_Rep &rep, void *ptr)
{
  ...
  if (pod)
    repSend(pod, Zdb_Msg::Rep, range, rep.op, db->config().compress);
}
...
Zdb_::Zdb_(ZdbEnv *env, ZdbID id, uint32_t version, ZdbHandler handler,
...
{
  {
    ...
    cacheLoads = m_cacheLoads;
    cacheMisses = m_cacheMisses;
  }
  {
    FSReadGuard guard(m_fsLock);
    fileLoads = m_fileLoads;
    fileMisses = m_fileMisses;
  }
}
...
void Zdb_::write_(ZdbRN rn, ZdbRN prevRN, const void *ptr, int op)
{
  int r;
  ZeError e;
  Zdb_FileRec rec;
...
void Zdb_::fileReadError_(Zdb_File *file, ZiFile::Offset off, int r, ZeError e)
{
  if (r < 0) {
    ZeLOG(Error, ZtString() <<
...
void Zdb_::fileWriteError_(Zdb_File *file, ZiFile::Offset off, ZeError e)
{
  ZeLOG(Error, ZtString() <<
