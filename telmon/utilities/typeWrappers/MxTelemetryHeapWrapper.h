...
protected:
    ...
    int fromChartPriorityToHeapIndex(const int a_index) const noexcept;
    // QPair(void*, int); void* = the type, int = to convert
    QPair<void*, int> getHeapDataType(void* const a_mxTelemetryMsg, const int a_index) const noexcept;

public:

    // must correspond to struct index
    enum ZmHeapSelemetryStructIndex {e_id, e_cacheSize, e_cpuset, e_cacheAllocs, e_heapAllocs,
                                     e_frees, e_size, e_partition, e_sharded, e_alignment};

    const QList<QString>& getTableList() const noexcept override final;
    const QVector<QString>& getChartList() const noexcept override final;
    const  std::array<int, 2>&  getActiveDataSet() const noexcept override final;
    bool isDataTypeNotUsed(const int a_index) const noexcept override final;
