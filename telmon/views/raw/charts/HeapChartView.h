...
#ifndef HEAPCHARTVIEW_H
#define HEAPCHARTVIEW_H

#include "BasicChartView.h"

class HeapChartView : public BasicChartView
{
public:
    HeapChartView(QChart *chart, const std::array<unsigned int, 2>&  a_activeDataList, QWidget *a_parent = nullptr);
    virtual ~HeapChartView() override;

    // should match ZmHeapTelemetry, ordered by priority, added none option for presentation
    enum ChartHeapTelemetry {size, alignment, partition, sharded, cacheSize,
