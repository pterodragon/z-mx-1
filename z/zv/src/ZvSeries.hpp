...
#pragma pack(pop)
  ...
  struct BufLRU_HeapID {
    static const char *id() { return "ZvSeries.BufLRU"; }
  };
  using BufLRU =
    ZmList<BufLRUNode_,
      ZmListObject<ZuNull,
  ...
  struct Buf_HeapID {
    static const char *id() { return "ZvSeries.Buf"; }
  };
  ...
  public:
    ...
    static Reader reader(const Series *s, uint64_t offset) {
      ZmRef<Buf> buf;
    ...
    static Reader index(const Series *s, int64_t value) {
      ZmRef<Buf> buf;
...
struct Impl : public Series<Impl, Buf> {
  ...
  public:
    Series(const ZvField *field) : m_field(field) { }
    ...
    const ZvField *field() const { return m_field; }
  ...
  public:
    ...
    template <typename BufReader>
    auto index(int64_t value) const {
  ...
  private:
    ...
    template <typename Reader>
    Reader firstIndex_(ZmRef<Buf> &buf, unsigned search, int64_t value) const {
    ...
    auto indexSearch(int64_t value) const {
    ...
    template <typename BufReader>
    BufReader firstIndex(ZmRef<Buf> &buf, int64_t value) const {
      return firstIndex_<BufReader>(buf,
	  ZuInterSearch(&m_blks[0], m_blks.length(), indexSearch(value)),
	  value);
    }
    template <typename BufReader>
    BufReader firstIdxFwd(ZmRef<Buf> &buf, int64_t value) const {
      return firstReader_<BufReader>(buf,
	  ZuInterSearch(
    ...
    template <typename BufReader>
    BufReader firstIdxRev(ZmRef<Buf> &buf, int64_t value) const {
      return firstReader_<BufReader>(buf,
  ...
  struct File_HeapID {
    static const char *id() { return "ZvSeries.File"; }
  };
  using FileHash =
    ZmHash<File_,
  ...
  class ZvAPI FileMgr : public Mgr {
  public:
    struct Config {
  ...
  friend Base;
    ...
    FileSeries(const ZvField *field) : Base{field} { }
    ...
    bool open(ZuString parent) {
      auto field = this->field();
      const char *id = field ? field->id() : "_index";
      return mgr()->open(this->seriesID(), parent, id, OpenFn{
